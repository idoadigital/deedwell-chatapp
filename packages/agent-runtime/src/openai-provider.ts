...
export const SCHEMA_HINTS: Record<ModelRequest["outputSchemaRef"], string> = {
  ...
  site_edit_plan: `Output JSON shape: {"understood":boolean,"clarification":string|null(a single question when the request cannot be carried out as asked),"title":string(<=60, version-history label, e.g. "Hero typography updated"),"summary":string(<=400, what you will change, present tense),"reply":string(<=600, the message to the customer once verified: what changed, where it was checked, what was left alone),"affectedPages":string[](page slugs),"viewports":integer[](widths in px to verify at, 1-4, e.g. [390,1440]),"operations":[ {"kind":"style","page":string|"*","selector":string(a CSS selector from page_map or a simple one like "#hero h1"),"viewport":"all"|"mobile"|"tablet"|"desktop","declarations":{cssProperty:string},"note":string} | {"kind":"section","page":string,"sectionId":string,"set":{"component"?:string,"variant"?:string,"background"?:"default"|"muted"|"surface"|"dark"|"primary"|"accent-tint","imagePosition"?:"none"|"left"|"right"|"background"|"top"|"full","image"?:string|null,"density"?:"airy"|"balanced"|"dense","motion"?:"none"|"fade-up"|"stagger"|"image-reveal"|"count"|"parallax","overrides"?:{"eyebrow"?:string,"heading"?:string,"body"?:string}}} | {"kind":"section-move","page":string,"sectionId":string,"position":integer} | {"kind":"section-remove","page":string,"sectionId":string} | {"kind":"section-add","page":string,"afterSectionId":string|null,"component":string,"variant"?:string,"background"?:string,"block":{"kind":"text","heading":string|null,"body":string}|integer,"purpose":string} | {"kind":"copy","page":string,"blockIndex":integer,"field":string(dot path such as "heading" or "items.0.description"),"value":string} | {"kind":"tokens","patch":{partial design tokens}} | {"kind":"page-cta","page":string,"primaryCta":{"label":string,"href":string}|null} ](0-12, the SMALLEST set that does the job),"expectations":[{"selector":string,"viewport":integer,"property":"fontSize"|"height"|"width"|"marginTop"|"marginBottom"|"paddingTop"|"paddingBottom"|"gap"|"lineHeight"|"top"|"color"|"backgroundColor"|"display","direction":"decrease"|"increase"|"equal"|"change"}](0-8, measurable outcomes to verify; include "equal" checks at other widths when a change must NOT spread)}. Prefer "style" with viewport "mobile" for phone-only presentation changes; use "copy" only for wording; use "section" for layout/variant/spacing/background; never invent facts about the organization.`,
  site_qa_review: `Output JSON shape: {"missionClear":boolean,"summary":string(<=400),"issues":[{"page":string(slug),"category":"content"|"mission"|"footer"|"navigation","severity":"critical"|"high"|"medium"|"low","title":string(<=120),"description":string(<=400),"quote":string|null(the exact page text at issue),"replacement":string|null(a corrected wording ONLY when the organization facts support it; otherwise null)}](0-30)}. Flag placeholders, unsupported claims, another organization's name, an unclear mission, inconsistent naming, duplicate or empty copy. Do not flag style preferences.`,
  ...
  logo_brief: `Output JSON shape: {"organizationName":string,"tagline":string|null,"description":string(<=1200 chars),"objectives":string[](1-12, what the logo must communicate, e.g. "trust","opportunity"),"audience":string,"personality":string[](1-12 adjectives, e.g. "Modern","Human","Warm"),"logoType":"wordmark"|"lettermark"|"emblem"|"symbol_wordmark"|"combination"|"ai_choice","visualStyle":string[](1-12, e.g. "Minimal","Geometric","Organic"),"colors":{"mode":"existing"|"suggested"|"custom","palette":["#rrggbb",...](1-6),"notes":string|null},"symbolism":string[](0-12 themes worth exploring),"avoid":string[](0-12 unwanted directions),"designerNotes":string(<=1200 chars, the creative direction)}.`,
  logo_concepts: `Output JSON shape: {"concepts":[{"title":string(<=80),"approach":string(<=80, e.g. "Typography-led","Abstract symbol","Geometric","Organic","Minimal emblem"),"logoType":"wordmark"|"lettermark"|"emblem"|"symbol_wordmark"|"combination","direction":string(<=900, concrete art direction: the mark's forms, the type treatment, the lockup)}]} with 5 concepts unless told otherwise (min 3, max 6).`,
  agent_task_result: `Output JSON shape: {"summary":string(<=1200, what was done, past tense, specific),"progressNotes":string[](2-6 short steps taken, in order),"deliverables":[{"title":string,"body":string(markdown, the complete deliverable)}](0-6, usually 1),"imageRequests":[{"title":string,"prompt":string(a full image prompt)}](0-4, only when the task asks for an image or design),"needsFromUser":string|null(a single question when the task cannot be done without information only the user has; otherwise null),"handoffs":[{"agentKey":string(a teammate agentKey from the roster),"title":string,"instructions":string}](0-4, ONLY when a distinct part of the task clearly belongs to another teammate's specialty; usually empty)}.`,
  task_schedule_reply: `Output JSON shape: {"recurring":boolean|null,"cron":string|null(five-field cron in the user's time zone when a cadence is stated),"runAt":string|null(ISO datetime when a one-off task should run later),"confirm":boolean|null(true for yes/confirm/go ahead/looks good, false for cancel/never mind/stop, else null),"changes":{"title":string|null,"instructions":string|null,"agentKey":string|null,"priority":"low"|"normal"|"high"|"urgent"|null,"requiresApproval":boolean|null}}.`,
  proactive_message: `Output JSON shape: {"message":string(<=900, the agent's chat message: specific, warm, no "just checking in", ends with the one thing the user can do or say),"summary":string(<=140, one line for a notification),"shouldSend":boolean(false when this would not materially help the user right now),"reason":string|null}.`,
  ...
  site_patch: `Output JSON shape: {"applied":boolean,"reason":string|null,"changeSummary":string,"pages":[same page shape as the input pages block],"designInstruction"?:string|null,"affectedPages"?:string[],"imageRequests"?:[{"key":string,"prompt":string}]}. Return the COMPLETE updated page set. Text changes go in "pages". A look-or-layout change (move an image, a different section style, spacing, colours) goes in "designInstruction" with the slugs it applies to in "affectedPages", and "applied" is true even if no text changed. A request to change a picture goes in "imageRequests" with the image key from site_images and a description of what it should show. If the request cannot be translated faithfully, set applied=false with a reason and return the pages unchanged.`,
  ...
  google_ads_strategy: `Output JSON shape: {"title":string,"objective":string,"audiences":[{"name":string,"description":string}],"themes":[{"name":string,"description":string}],"campaigns":[{"name":string,"objective":string,"landingPage":string(url from the supplied website pages),"keywordThemes":string[],"adGroupIdeas":string[]}],"landingPages":[{"url":string,"purpose":string,"recommendations":string[]}],"keywordThemes":[{"theme":string,"examples":string[]}],"negativeKeywords":string[],"conversionGoals":[{"name":string,"howToTrack":string}],"budget":{"monthlyUsd":number,"rationale":string}|null,"opportunities":string[],"adGrantsNotes":string[]}. Ground everything in the supplied organization context; never invent programs, outcomes or URLs.`,
  google_ads_campaign_draft: `Output JSON shape: {"name":string,"objective":string,"landingPage":string(url),"dailyBudgetMicros":integer(1 USD = 1000000),"geoTargets":string[],"negativeKeywords":string[],"adGroups":[{"name":string,"keywords":[{"text":string,"matchType":"BROAD"|"PHRASE"|"EXACT"}](5-15),"negativeKeywords":string[],"ads":[{"title":string,"headlines":string[](8-15, each <=30 chars, all distinct),"descriptions":string[](3-4, each <=90 chars, all distinct),"finalUrl":string(url),"path1":string|null(<=15 chars, no spaces),"path2":string|null,"rationale":string}](>=2)}],"sitelinks":[{"linkText":string(<=25 chars),"description1":string(<=35 chars),"description2":string(<=35 chars),"finalUrl":string(url)}](2-6),"callouts":string[](4-8, each <=25 chars),"imageCreatives":[{"title":string,"prompt":string(a detailed photographic brief, no text or logos in the picture),"altText":string}](1-2),"rationale":string}. Character limits are hard limits. Use only landing pages and facts from the supplied context.`,
  google_ads_request_plan: `Output JSON shape: {"decision":"proceed"|"needs_info"|"decline_recommended","customerSummary":string(plain words for the nonprofit, <=1500 chars),"adminSummary":string(for the Deedwell administrator, <=3000 chars),"assessment":{"missionRelevance":string,"eligibility":string[],"policyChecks":[{"rule":string,"status":"pass"|"warning"|"fail"|"unknown","evidence":string}],"landingPage":{"url":string|null,"ready":boolean,"findings":string[]},"risks":string[]},"questions":[{"question":string,"why":string,"audience":"customer"|"admin"}](only inputs that block eligibility, targeting, measurement, claims or authorization; required when decision is needs_info; audience "admin" when the Deedwell administrator, not the nonprofit, must answer),"measurement":[{"name":string,"meaning":string,"source":string,"role":"primary"|"secondary","notes":string}],"utilization":{"suggestedDailyBudgetUsd":number|null,"bindingConstraint":string,"notes":string[]},"plan":null|{same shape as a google_ads_strategy: "title","objective","audiences","themes","campaigns":[{"name","objective","landingPage"(url from the supplied website pages),"keywordThemes","adGroupIdeas"}],"landingPages","keywordThemes","negativeKeywords","conversionGoals","budget","opportunities","adGrantsNotes"}(required when decision is proceed),"nextSteps":string[],"declineReason":string|null(required when decision is decline_recommended)}. Ground everything in the supplied context; mark unknown facts UNKNOWN rather than inventing them; never claim anything was published, uploaded or verified.`,
  content_strategy: `Output JSON shape: {"audience":string,"message":string,"tone":string,"palette":string,"designs":[{"caption":string,"prompt":string,"postText":string}]} with BETWEEN 4 AND 6 designs. Each design's prompt is a complete, self-contained image-generation prompt that already obeys the design guidelines in your instructions, and must state the exact words to appear in the image in quotes. Each design's postText is the ready-to-post social caption for that image: a scroll-stopping first line, one or two sentences in the organization's voice, a call to action, then 3-8 specific hashtags on the last line; 300-900 characters, plain text. Ground every choice in the organization context — never invent statistics, dollar amounts, dates, or outcomes that are not in it.`,
...
{"action":"create_content","kind":"social"|"flyer"|"buying_guide"|"event_promo","prompt":string} — designed images: social posts, flyers, guides, event graphics
{"action":"create_task","title":string,"instructions":string|null,"agentKey":string|null(a teammate agentKey from context.teammates),"priority":"low"|"normal"|"high"|"urgent"|null,"recurring":boolean|null(null when the user did not say one-time vs recurring),"cron":string|null(five-field cron when a cadence was stated, e.g. "0 9 * * 1" for Mondays 9am),"runAt":string|null(ISO datetime for a one-off later run),"requiresApproval":boolean|null} — the user wants a teammate to take on a task, chore, routine, or scheduled job ("every Monday…", "each month…", "set up a task to…", "have Grace draft…")
